...
class FunctionWorker:
    ...
    # both fx receives instance_pid and timestamp_map as kwargs
    def _fork(self, parent_fx=None, child_fx=None):
        try:
            ...
            if instance_pid == 0:
                if child_fx is not None:
                    return instance_pid, timestamp_map, child_fx(instance_pid=instance_pid, timestamp_map=timestamp_map), None
            else:
                if parent_fx is not None:
                    return instance_pid, timestamp_map, parent_fx(instance_pid=instance_pid, timestamp_map=timestamp_map), None

            return instance_pid, timestamp_map, None, None
        
        except Exception as exc:
            if instance_pid == 0:
                self._logger.exception("Child exception: %s", str(exc))
                return instance_pid, timestamp_map, None, exc
            else:
    ...
    def _handle_request(self, key, encapsulated_value, timestamp_map = {}):
    ...
    def _handle_request_and_exit(self, key, encapsulated_value, timestamp_map = {}):
        exc = self._handle_request(key, encapsulated_value, timestamp_map)
        if exc is Exception:
            os._exit(1)
        else:
            os._exit(0)
    ...
    def _fork_and_handle_message(self, key, encapsulated_value):
        def pfx(instance_pid, timestamp_map):
            # parent
            ...
            return None
        def cfx(instance_pid, timestamp_map):
            self._handle_request_and_exit(key, encapsulated_value, timestamp_map)
            
        self._fork(parent_fx=pfx, child_fx=cfx)
